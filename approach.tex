...
A popular software suite called TweetNLP~\cite{owoputi2013improved,kong2014dependency} is used to generate such syntactical tags for each token given a tweet.
Given the nature of the noisy tweet text, a new set of POS tags and dependency roles are used in the tool, called Tweebank~\cite{gimpel2011part}. Since a tweet often contains more than one utterance, the output of TweeboParser will often be a multi-rooted graph over the tweet. Also, many elements in tweets have no syntactic function. These include, in many cases, hashtags, URLs, and emoticons. It tries to exclude these tokens from the parse tree.
